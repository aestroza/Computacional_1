...
sed "s/\([0-9]*\)\(\.\)\([0-9]*\)/\1\.\3/g" datos_sismos.dat > gatos_sismos_puntos.dat

#para comparar es mas limpio tener las horas minutos y segundos en numero,0 en vez de numero,

sed "s/\([0-9]*\)\(\,\)\(\s\)/\1\.0 /4g" gatos_sismos_comas.dat > horas_gatos_sismos_puntos.dat
...
awk -f extraer_septiembre_entre_8am_y_12am.awk horas_gatos_sismos_puntos.dat > septiembre_2010_entre_8am_y_12am.dat
